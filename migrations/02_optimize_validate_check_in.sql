-- 向上迁移 (应用更改)
-- 备份原始函数
CREATE OR REPLACE FUNCTION validate_check_in_backup() 
RETURNS trigger AS $$ 
BEGIN 
  -- 复制原始函数的内容
  RETURN NEW; 
...
Member not found';
  ...
  -- 检查是否重复签到
  IF check_duplicate_check_in(NEW.member_id, NEW.check_in_date, NEW.class_type) THEN
...
Membership card does not belong to this member';
    ...
    NEW.is_extra := CASE
      -- 卡类型不匹配
      WHEN (v_card.card_type = 'group' AND NEW.class_type = 'private') OR 
           (v_card.card_type = 'private' AND NEW.class_type != 'private') THEN true
      -- 卡已过期
