...
DECLARE
  v_check_in_id uuid;
  v_is_extra boolean := true; -- Default to true. This is the failsafe.
...
BEGIN
  ...
  -- 4. Validate the found card. If no card, this block is skipped.
  -- The default message will be '未找到可用的会员卡'.
  v_message := '未找到可用的会员卡';
  ...
  IF v_matched_card_id IS NOT NULL THEN
    ...
    IF (v_validity->>'is_valid')::boolean THEN
      -- This is the ONLY path to a successful, non-extra check-in
      v_is_extra := false;
      v_message := '签到成功';
    ELSE
      -- Card was found but is invalid (expired, wrong type, etc.)
      v_message := v_validity->>'reason';
    END IF;
  END IF;
  
  -- 5. Insert check-in record using the determined state
