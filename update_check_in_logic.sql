-- Main check-in function with automated card matching for private classes.
-- It now uses the `find_matching_card` helper function.
...
DECLARE
  ...
  v_message text;
  v_validity jsonb;
  v_matched_card_id uuid; -- New variable to store the correctly matched card ID
BEGIN
  ...
  IF v_is_private THEN
    -- For private classes, we find the best matching card automatically
    v_matched_card_id := find_matching_card(p_member_id, p_class_type, p_trainer_id);
  ELSE
    -- For other class types (e.g., group), we rely on the provided p_card_id
