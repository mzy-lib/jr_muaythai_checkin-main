...
DECLARE
  v_check_in_id uuid;
  v_is_extra boolean := true; -- CRITICAL FIX: Default to true
...
BEGIN
  ...
  IF v_matched_card_id IS NOT NULL THEN
    ...
    IF (v_validity->>'is_valid')::boolean THEN
      v_is_extra := false; -- Only set to false if a valid card is found and verified
      v_message := '签到成功';
    ELSE
      v_message := v_validity->>'reason'; -- Card found but invalid, will be an extra check-in
    END IF;
  ELSE
    v_message := '未找到可用的会员卡'; -- No card found, will be an extra check-in
  END IF;
