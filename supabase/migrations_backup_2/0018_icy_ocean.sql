-- Enhanced check-in processing with improved monthly membership handling
...
BEGIN
  ...
  IF v_membership IN ('single_daily_monthly', 'double_daily_monthly') THEN
    ...
    IF v_membership_expiry < CURRENT_DATE THEN
      NEW.is_extra := true;
    ELSE
      -- Count only non-extra check-ins for today
      SELECT COUNT(*)
      ...
      -- Set is_extra based on daily limit
      IF v_membership = 'single_daily_monthly' THEN
        NEW.is_extra := v_daily_check_ins >= 1;
      ELSE -- double_daily_monthly
...
-- Fix member search to handle special characters
