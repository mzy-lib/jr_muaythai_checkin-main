-- Delete any existing check-ins for test member
...
-- Add improved error handling for check-in validation
...
BEGIN
  ...
  -- New members are always marked as extra check-ins
  IF v_is_new_member OR v_membership IS NULL THEN
    NEW.is_extra := true;
    RETURN NEW;
  END IF;
