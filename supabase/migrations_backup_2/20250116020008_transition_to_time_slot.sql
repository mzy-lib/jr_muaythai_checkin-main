-- Transition to time_slot
BEGIN;

-- 1. Create new version of register_new_member that works with time_slot
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing_check_in.is_private THEN
      RAISE EXCEPTION '今天已经签到过该时段的私教课。Already checked in for this time slot today.'
