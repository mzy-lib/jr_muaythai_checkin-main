-- 修复validate_check_in函数中的过期会员卡检测逻辑（第二版）
-- 创建于: 2025-03-02
-- 作者: Claude AI
...
VALUES (
    'fix_validate_check_in_function_v2',
    '开始修复validate_check_in函数（第二版）',
    jsonb_build_object(
        'timestamp', NOW(),
        'issue', '过期会员卡检测逻辑错误',
        'description', '测试显示过期会员卡签到仍未被正确标记为额外签到'
    )
);
...
BEGIN
    -- 查看测试中使用的会员卡
    SELECT * INTO v_test_card
    FROM membership_cards
    WHERE id = '6a47a6a2-6a48-4bed-9a50-983bed19edec';
    
    RAISE NOTICE '测试会员卡信息: valid_until = %', v_test_card.valid_until;
    
    -- 检查valid_until是否为NULL
    IF v_test_card.valid_until IS NULL THEN
        RAISE NOTICE '问题发现: 测试会员卡的valid_until为NULL，导致过期检测失败';
    END IF;
END $$;
...
VALUES (
    'fix_validate_check_in_function_v2',
    '完成修复validate_check_in函数（第二版）',
    jsonb_build_object(
        'timestamp', NOW(),
        'changes', '修复过期会员卡检测逻辑，确保正确处理NULL值'
    )
);

-- 添加测试函数，用于验证修复后的函数是否正常工作
CREATE OR REPLACE FUNCTION test_expired_card_validation_v2()
RETURNS VOID AS $$
DECLARE
    v_member_id UUID;
    v_card_id UUID;
...
-- 执行测试函数
SELECT test_expired_card_validation_v2();

-- 授予函数执行权限
GRANT EXECUTE ON FUNCTION validate_check_in() TO postgres, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION test_expired_card_validation_v2() TO postgres, anon, authenticated, service_role;
